...
# NOTE: get this to work before you add in environmental variables
...
peng.ipm.1 <- nimbleCode({
  ...
  N1[1] ~ T(dnorm(6500, 4e-08),0,)
  Nad[1] ~ T(dnorm(190000, 5e-08),0,)
  ...
  sigma.y ~ dunif(0, 200000) # (sd) changed from 50000
...
initial.values <- function(){list(l.mphij = rnorm(1, 0.2, 0.5),
                                  ...
                                  Nad = rpois(dim(CH.J.marray)[2], 190000))} #changed from 200000
...
ggsave("projected_counts_11.12.21.png", dpi = 600, width = 12, height = 10)


##################################################################################################################

#### ADDING ENVIRONMENTAL VARS ####

peng.ipm.2 <- nimbleCode({
  ...
  # covariates
  for (i in 1:3){alpha[i] ~ dnorm(0, 0.34)}
  ...
  for (t in 1:(nyears-1)){
    logit(phi.juv[t]) <- l.mphij
    logit(phi.ad[t]) <- l.mphia
    log(fec[t]) <- l.mfec + (alpha[1] * rain[t]) + (alpha[2] * temp[t]) + (alpha[3] * ssta_m[t])
    #logit(pj[t]) <- l.pj
    #logit(pa[t]) <- l.pa
  }
...
my.constants <- list(nyears = dim(CH.J.marray)[2],
                     ...
                     ssta_m = as.vector(c(0, ssta_migration$mean.2[1:37]))) # lagged SSTA!
...
# run code!
peng.ipm2.out <- nimbleMCMC(code = peng.ipm.2,
                            constants = my.constants,
                            data = my.data,
                            inits = initial.values,
                            monitors = c("mphij", "mphia", "mpj", "mpa", 
...
MCMCplot(peng.ipm2.out, params = 'alpha',
         labels = c("Rain", "Temp", "SSTA-m"))

# extract data and calculate means/95% CIs to make graphs
alpha <- c(peng.ipm2.out$chain1[,'alpha[1]'], peng.ipm2.out$chain2[,'alpha[1]'], peng.ipm2.out$chain3[,'alpha[1]'])
fec <- c(peng.ipm2.out$chain1[,'mfec'], peng.ipm2.out$chain2[,'mfec'], peng.ipm2.out$chain3[,'mfec'])
predicted_fec <- matrix(NA, nrow = length(alpha), ncol = length(my.constants$rain))
for (i in 1:length(alpha)){
  for (j in 1:length(my.constants$rain))
    predicted_fec[i,j] <- fec[i] + alpha[i] * my.constants$rain[j] 
}

mean_fec <- apply(predicted_fec, 2, mean)
lci_fec <- apply(predicted_fec, 2, quantile, prob = 2.5/100)
uci_fec <- apply(predicted_fec, 2, quantile, prob = 97.5/100)
ord <- order(my.constants$rain)
df <- data.frame(rain = c(my.constants$rain[ord]),
                 fec = mean_fec[ord],
                 lci = lci_fec[ord],
                 uci = uci_fec[ord])
...
df %>%
  ...
  labs(x = "Standardized Total Rain b/t 15Oct to 15Dec", y = "Estimated Fecundity", color = "", fill = "")

# convert back to normal units
df$real_rain <- (df$rain*30.54473)+36.61667
df %>%
  ggplot() + 
  aes(x = real_rain, y = fec) +
  geom_line()+
  geom_ribbon(aes(ymin = lci, ymax = uci), alpha = 0.5)+
  labs(x = "Total Rain b/t 15Oct to 15Dec", y = "Estimated Fecundity", color = "", fill = "")
ggsave("repro_rain_predicted_11.15.1.png",dpi=600,width=10)

##################################################################################################################

#### ADDING ALL ENVIRONMENTAL VARS ####

peng.ipm.3 <- nimbleCode({
  ...
  # covariates
  for (i in 1:9){alpha[i] ~ dnorm(0, 0.34)}
...
# run code!
peng.ipm3.out <- nimbleMCMC(code = peng.ipm.3,
                            constants = my.constants,
                            data = my.data,
                            inits = initial.values,
                            monitors = c("mphij", "mphia", "mpj", "mpa", 
...
#ggsave("ipm_param_results_12.2.21.png", dpi = 600)
